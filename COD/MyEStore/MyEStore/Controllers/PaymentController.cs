...
namespace MyEStore.Controllers
{
    ...
    public class PaymentController : Controller
    {
        ...
        public IActionResult Checkout(CheckoutVM model)
        {
            ...
            var gioHangSession = CartItems; // Giả định bạn có thuộc tính CartItems để lấy giỏ hàng từ session
            if (gioHangSession == null || !gioHangSession.Any())
            {
                return RedirectToAction("Index", "Cart"); // Không có sản phẩm trong giỏ hàng
            }

            // Lấy giỏ hàng từ DB
            var existingCartItems = _context.Carts
                .Where(c => c.MaKh == maKh)
                .Include(c => c.MaHhNavigation) // Eager loading
                .ToList();

            // Duyệt qua các sản phẩm trong session
            foreach (var item in gioHangSession)
            {
                // Kiểm tra nếu sản phẩm đã tồn tại trong giỏ hàng DB
                var existingItem = existingCartItems.SingleOrDefault(c => c.MaHh == item.MaHh);
            ...
            // Xóa giỏ hàng trong session sau khi đã lưu vào DB
            CartItems = new List<CartItem>();

            return RedirectToAction("Index", "Cart"); // Trở lại trang giỏ hàng
        }
